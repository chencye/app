package com.github.chencye.app.common;
...
public final class ReflectUtils {
     ...
     * 
     * @param from
     *            源，对其各字段判断空。若非空，判断是否需要深层拷贝，不需要则输出字段值
     * @param to
     *            目标，接收from不为空的字段值
     * @param selfPackName
     *            自定义类包名，这种包下的类，会进行深层拷贝。为null时，不进行深层拷贝
     * @param isDiffCopy
     *            是否拷贝 非null且非equals字段
     */
    ...
    public static <T> void copyPropertiesOnNotNull(T from, T to) {
        // 取from前三级包名为自定义类型
        String selfPackName = StringUtils.getRootPackName(from.getClass(), 3);
        copyProperties(from, to, selfPackName, true);
    }
     ...
     *            源，输出相应字段值
     * @param to
     *            目标，对其各字段判断空：为空，则接收from中相应字段值；不为空，进行深层拷贝
     * @param selfPackName
     *            自定义类包名，这种包下的类，会进行深层拷贝
     */
